...
AnimElement::AnimElement(const char* path) : gif(std::make_unique<GIFSprite>(fopen(path, "r"))){
	lastMicros = micros();
}
...
void AnimElement::setLoopDoneCb(std::function<void()> cb){
	if(gif == nullptr){
		return;
	}

	gif->setLoopDoneCallback([cb](uint32_t){ cb(); });
}
...
void AnimElement::draw(Sprite* canvas){
	...
	sprite.pushRotateZoom(canvas, std::round(getX() + (float) sprite.width() / 2.0), std::round(getY() + (float) sprite.height() / 2.0), 0, 1, 1, TFT_TRANSPARENT);
}
