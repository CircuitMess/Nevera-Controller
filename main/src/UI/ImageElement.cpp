...
ImageElement::ImageElement(const char* path, uint16_t width, uint16_t height) : width(width), height(height){
	file = fopen(path, "r");
	if(file == nullptr){
...
void ImageElement::draw(Sprite* canvas){
	...
	drawFile(*canvas, file, getX(), getY(), width, height, 1, TFT_TRANSPARENT);
}
